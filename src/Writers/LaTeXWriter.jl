...
function latex(io::IO, code::Markdown.Code)
    language = if code.language == "jldoctest"
        ...
        # as the language attribute. The check here to determine if it is a REPL-type or
        # script-type doctest should match the corresponding one in DocChecks.jl, to make
