...
function mdconvert(c::Markdown.Code, parent::MDBlockContext)
    ...
    elseif c.language == "jldoctest"
        # In case doctests are not being run, the Markdown.Code blocks will have jldoctest
        # as the language attribute. The check here to determine if it is a "console"-type
        # or "code"-type doctest should match the corresponding one in DocChecks.jl, to make
